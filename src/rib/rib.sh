...
-s <value>, --ssh <value>, --ssh=<value>
    Absolute path of private .ssh key to mount into RiB Container. Needed for AWS/T&E
    RiB functionality
-h, --help
    Print help and exit
...
--network-host
    Set the RiB network to host instead of rib-overlay-network. This is required for
    T&E mode due to local SSH, but will break aws mode and local mode
--detached
    Run RiB Detached
--dev
...
# Gitlab
CONTAINER_REGISTRY="gitlab.race.twosixlabs.com:4567"
CONTAINER_REPO="race-common/race-in-the-box"
...
do
    ...
    case $key in
        ...
        RUN_UI=false
        formatlog "WARNING" "LOCAL Mode will not work, only set for T&E Mode"
        shift
        ;;
        --ui)
        RUN_UI=true
...
if [ "$RUN_UI" = true ] ; then
    ...
    docker run --rm -d --network rib-overlay-network -p 8501:80 --name=race-in-the-box-ui "${UI_IMAGE}"
fi
