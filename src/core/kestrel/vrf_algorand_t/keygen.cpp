...
void vrf_algorand_t::keygen(unsigned char * const pk,
                            unsigned char * const sk) const {
  SST_ASSERT((pk != nullptr));
  SST_ASSERT((sk != nullptr));
  SST_ASSERT((!sst::overlap(pk, pk_size(), sk, sk_size())));
  int const s = sodium::crypto_vrf_keypair(pk, sk);
  if (s != 0) {
