...
#ifndef KESTREL_EASY_TA2_PLUGIN_T_DEACTIVATECHANNEL_HPP
#define KESTREL_EASY_TA2_PLUGIN_T_DEACTIVATECHANNEL_HPP
...
PluginResponse easy_ta2_plugin_t<Plugin, Types>::deactivateChannel(
    RaceHandle const handle,
    std::string const channelGid) {
  tracing_event_t SST_TEV_DEF(tev);
  SST_TEV_ADD(tev,
              "plugin_function_input",
              this->i_deactivateChannel(handle, channelGid));
  return this->template inner_call<func_deactivateChannel,
                                   PLUGIN_ERROR>(
      tev,
      [&](tracing_event_t & tev) {
        this->expect_active();
        this->verify_channel_id(channelGid);
        this->inner_deactivateChannel(SST_TEV_ARG(tev));
        ...
        SST_TEV_ADD(tev,
                    ...
                    this->o_deactivateChannel(handle, channelGid));
      });
}

} // namespace kestrel

#endif // #ifndef KESTREL_EASY_TA2_PLUGIN_T_DEACTIVATECHANNEL_HPP
