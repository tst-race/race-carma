...
void c_ta2_plugin_t::inner_activateChannel(
    tracing_event_t tev,
  ...
  SST_TEV_ADD(tev);
  try {
    plugin_response_t const r(plugin().activateChannel(
        plugin().plugin,
        handle.value(),
        reinterpret_cast<kestrel_char const *>(
            channelGid.value().c_str()),
        reinterpret_cast<kestrel_char const *>(roleName.c_str())));
    if (r != plugin_response_t::ok()) {
      throw std::runtime_error("activateChannel() failed");
    }
  }
  SST_TEV_RETHROW(tev);
}
