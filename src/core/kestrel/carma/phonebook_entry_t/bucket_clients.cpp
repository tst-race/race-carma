...
void phonebook_entry_t::clear_bucket_clients() const {
  SST_ASSERT((!moved_from_));
  bucket_clients_.store(nullptr);
}

void phonebook_entry_t::parse_bucket_clients(json_t & src) {
  SST_ASSERT((!moved_from_));
  if (bucket_clients_.load()) {
    return;
  }
  parse_role(src);
  ...
  parse_phonebook_set(src, "bucket_clients", bucket_clients_);
}

void phonebook_entry_t::unparse_bucket_clients(json_t & dst) const {
  SST_ASSERT((!moved_from_));
  SST_ASSERT((dst.is_object()));
  unparse_phonebook_set(dst, "bucket_clients", bucket_clients_);
}

phonebook_set_t const &
phonebook_entry_t::bucket_clients(tracing_event_t tev) const {
  SST_TEV_TOP(tev);
  SST_ASSERT((!moved_from_));
  SST_ASSERT(
      (role() == role_t::client() || role() == role_t::mb_server()));
  phonebook_set_t * val = bucket_clients_.load();
  if (!val) {
    ...
    for (phonebook_pair_t const & pair : phonebook()) {
      ...
      if (entry->role() == role_t::client()
          && entry->bucket(SST_TEV_ARG(tev)) == b) {
        new_val->emplace(&pair);
      }
    }
    bucket_clients_.compare_exchange_strong(val, std::move(new_val));
  }
  return *val;
  SST_TEV_BOT(tev);
}
