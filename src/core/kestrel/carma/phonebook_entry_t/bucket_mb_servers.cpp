...
void phonebook_entry_t::clear_bucket_mb_servers() const {
  SST_ASSERT((!moved_from_));
  bucket_mb_servers_.store(nullptr);
}

void phonebook_entry_t::parse_bucket_mb_servers(json_t & src) {
  SST_ASSERT((!moved_from_));
  if (bucket_mb_servers_.load()) {
    return;
  }
  parse_role(src);
  ...
  parse_phonebook_set(src, "bucket_mb_servers", bucket_mb_servers_);
}

void phonebook_entry_t::unparse_bucket_mb_servers(json_t & dst) const {
  SST_ASSERT((!moved_from_));
  SST_ASSERT((dst.is_object()));
  unparse_phonebook_set(dst, "bucket_mb_servers", bucket_mb_servers_);
}

phonebook_set_t const &
phonebook_entry_t::bucket_mb_servers(tracing_event_t tev) const {
  SST_TEV_TOP(tev);
  SST_ASSERT((!moved_from_));
  SST_ASSERT(
      (role() == role_t::client() || role() == role_t::mb_server()));
  phonebook_set_t * val = bucket_mb_servers_.load();
  if (!val) {
    sst::unique_ptr<phonebook_set_t> new_val{sst::in_place};
    node_count_t const b = bucket(SST_TEV_ARG(tev));
    for (phonebook_pair_t const & pair : phonebook()) {
      ...
      if (entry->role() == role_t::mb_server()
          && entry->bucket(SST_TEV_ARG(tev)) == b) {
        new_val->emplace(&pair);
      }
    }
    bucket_mb_servers_.compare_exchange_strong(val, std::move(new_val));
  }
  return *val;
  SST_TEV_BOT(tev);
}
